...
Function vbs_array_to_json(v, Optional indent As String = "") As String
    ...
    For i = LBound(v) To UBound(v)
        json = json & indent & "  " & vbs_val_to_json(v(i), indent & "  ")
...
Function vbs_array_2d_to_json(v, Optional indent As String = "") As String
    ...
    Dim json As String, i As Long, j As Long, n As Long, m As Long
    n = UBound(v) - LBound(v) + 1
    m = UBound(v, 2) - LBound(v, 2) + 1
    
    For i = LBound(v) To UBound(v)
        json = json & indent & "[" & vbNewLine
        For j = LBound(v, 2) To UBound(v, 2)
            json = json & indent & "  " & vbs_val_to_json(v(i, j), indent & "  ")
            json = json & IIf(j = UBound(v, 2), "", ",") & vbNewLine
        Next j
        json = json & indent & "]"
        json = json & IIf(i = UBound(v), "", ",") & vbNewLine
    Next i
...
Function vbs_dict_to_json(d As Dictionary, Optional indent As String = "") As String
    ...
    For Each k In keys
        ...
        json = json & indent & "  " & vbs_json_string("" & k) & ": " & vbs_val_to_json(v, indent & "  ")
